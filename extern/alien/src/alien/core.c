...
static int alien_function_call(lua_State *L) {
  ...
  int *refi_args=0; /* VC runtime error checks catch if this is used but not initialized */
  int nrefi, nrefui, nrefd, nrefc;
  double *refd_args = 0; /* VC runtime error checks...*/
  char *refc_args = 0; /* VC runtime error checks...*/
  unsigned int *refui_args=0; /* VC runtime error checks...*/
  void **args=0; /* VC runtime error checks...*/
  ffi_cif *cif;
  alien_Function *af = alien_tofunction(L, 1);
  cif = &(af->cif);
  nparams = af->nparams;
