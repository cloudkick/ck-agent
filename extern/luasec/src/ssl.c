...
static const char *ssl_ioerror(void *ctx, int err)
{
  ...
  return lsec_socket_strerror(err);
}
...
static int meth_destroy(lua_State *L)
{
  ...
  if (ssl->ssl) {
    lsec_socket_setblocking(&ssl->sock);
    SSL_shutdown(ssl->ssl);
    lsec_socket_destroy(&ssl->sock);
    SSL_free(ssl->ssl);
    ssl->ssl = NULL;
  }
  return 0;
...
static int handshake(p_ssl ssl)
{
  ...
  for ( ; ; ) {
    ...
    case SSL_ERROR_WANT_READ:
      err = lsec_socket_waitfd(&ssl->sock, WAITFD_R, tm);
      if (err == IO_TIMEOUT) return IO_SSL;
      if (err != IO_DONE)    return err;
      break;
    case SSL_ERROR_WANT_WRITE:
      err = lsec_socket_waitfd(&ssl->sock, WAITFD_W, tm);
      if (err == IO_TIMEOUT) return IO_SSL;
      if (err != IO_DONE)    return err;
      break;
...
static int meth_setfd(lua_State *L)
{
  ...
  lsec_socket_setnonblocking(&ssl->sock);
