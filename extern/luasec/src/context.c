...
/**
 * Load trusted certificate from memory in PEM format
 */
static int load_cert_from_memory(lua_State *L)
{
  int rv;
  X509 *x = NULL;
  BIO *mem = NULL;
  ...
  mem = BIO_new_mem_buf((void*)cert, strlen(cert));
  
  /* cert -> x509 */
  x = PEM_read_bio_X509(mem, NULL, 0, NULL);
  if (x == NULL) {
    lua_pushboolean(L, 0);
    lua_pushfstring(L, "error parsing cert: (%s)",
                    ERR_reason_error_string(ERR_get_error()));
    BIO_free(mem);
    return 2;
  }

  rv = X509_STORE_add_cert(xstore, x);
  if (rv != 1) {
    lua_pushboolean(L, 0);
    lua_pushfstring(L, "error adding cert to x509 store (%s)",
                    ERR_reason_error_string(ERR_get_error()));
    BIO_free(mem);
    return 2;
  }
...
static luaL_Reg funcs[] = {
  ...
  {"loadcert_mem", load_cert_from_memory},
  {"loadcert",   load_cert},
  {"loadkey",    load_key},
